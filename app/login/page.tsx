  ...
  return (
    <div className="space-y-2">
      ...
      >
        ...
        <span>{isPending ? "Connecting to Google..." : "Sign in with Google"}</span>
      </button>
...
function PhoneOTPForm() {
  ...
  function sendOTP(e: React.FormEvent) {
    ...
    if (formatted.length < 13) { setError("Please enter a valid 10-digit mobile number"); return; }
    startTransition(async () => {
      const supabase = createClient();
  ...
  function verifyOTP(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      if (error) { setError("Wrong OTP. Please try again."); return; }
    ...
    return (
      <form onSubmit={verifyOTP} className="card p-6 space-y-5">
        ...
        <div className="flex flex-col items-center gap-2 pb-2">
          ...
          <p className="text-sm font-medium text-ink-800">OTP भेजा गया / OTP Sent</p>
        ...
        <div>
          <label className="block text-sm font-medium text-ink-700 mb-2 text-center">Enter 6-digit OTP</label>
          <input
            type="number"
            inputMode="numeric"
            pattern="[0-9]*"
  ...
  return (
    <form onSubmit={sendOTP} className="card p-6 space-y-5">
      <div>
        ...
        <p className="text-sm text-ink-400 text-center">अपना मोबाइल नंबर डालें / Enter your mobile number</p>
      </div>
...
function EmailLoginForm() {
  ...
  const [error, setError] = useState(
    urlError === "auth_callback_failed"
      ? "Verification link expired. Please sign in again."
      : ""
  );
    ...
    <form onSubmit={handleSubmit} className="card p-5 space-y-4">
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          ...
          <Link href="/forgot-password" className="text-xs text-sand-500 hover:text-sand-400 transition-colors">Forgot?</Link>
        </div>
        <div className="relative">
          ...
          <button type="button" onClick={() => setShowPassword((v) => !v)}
            className="absolute right-3 top-1/2 -translate-y-1/2 text-ink-300 hover:text-ink-500 transition-colors" aria-label={showPassword ? "Hide" : "Show"}>
    ...
    <div className="min-h-screen bg-cream-50 flex flex-col items-center overflow-y-auto px-5 py-10">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-ink-400 mt-1">वारंटी वॉलेट · Warranty Wallet</p>
        </div>
        ...
        <div className="text-center mt-5 space-y-3">
          <button onClick={() => setShowEmail((v) => !v)}
            ...
            {showEmail ? "← Use mobile number instead" : "Use email & password instead"}
          </button>
