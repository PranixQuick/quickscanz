...
export function usePushNotifications() {
  ...
  const [vapidReady] = useState(!!VAPID_PUBLIC_KEY);
  ...
  async function subscribe(): Promise<boolean> {
    ...
    if (!VAPID_PUBLIC_KEY) {
      toast("Push alerts coming soon — email notifications are active now!", { icon: "🔔", duration: 4000 });
      return false;
    }
    try {
      ...
      if (res.ok) { setIsSubscribed(true); setPermission("granted"); return true; }
      return false;
    } catch (err) {
  ...
  return { permission, isSubscribed, isSupported, vapidReady, subscribe, unsubscribe, requestPermission };
}
...
export default function NotificationSettings({ userId: _userId }: NotificationSettingsProps = {}) {
  const { permission, isSubscribed, isSupported, vapidReady, subscribe, unsubscribe, requestPermission } = usePushNotifications();
  ...
  return (
    <div className="card p-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-ink-400">
              {isSubscribed ? "Enabled — you'll be notified before warranties expire"
                : permission === "denied" ? "Blocked in browser settings"
                : !vapidReady ? "Coming soon — email alerts are active"
      ...
      {!vapidReady && !isSubscribed && (
        <p className="mt-3 pt-3 border-t border-cream-100 text-xs text-ink-300">
          📧 Email warranty alerts are active at your registered address.
        </p>
      )}
