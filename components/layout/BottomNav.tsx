        ...
        {NAV.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}
              ...
              {item.icon(active)}
              {!item.special && (
                <span className={`text-[9px] font-medium transition-colors text-center leading-tight line-clamp-2 max-w-[72px] ${active ? "text-ink-900" : "text-ink-300"}`} title={t(item.key)}>
                  {t(item.key)}
                </span>
              )}
            </Link>
