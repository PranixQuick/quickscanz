-- ============================================================
-- QuickScanZ Bootstrap Migration
-- Fixes:
--   1. Create profiles table (was missing → 42P01 error)
--   2. Create app_config table (replaces ALTER DATABASE SET
--      which needs superuser and fails in Supabase SQL editor)
--   3. Rewrite warranty cron to read config from app_config
--      table instead of database GUC parameters
-- ============================================================

-- ── 1. PROFILES TABLE ────────────────────────────────────────
-- Stores public user profile data. Mirrors auth.users.id.
...
-- Backfill emails for any existing users who signed up before profiles existed
insert into public.profiles (id, email)
...
-- ── 2. APP_CONFIG TABLE ──────────────────────────────────────
-- Replaces ALTER DATABASE SET app.* which requires superuser.
-- The cron function reads config values from here instead.
create table if not exists public.app_config (
  key   text primary key,
  value text not null
);

-- Seed the two config values needed by the push cron
-- Update these values here any time via SQL editor (no superuser needed)
insert into public.app_config (key, value) values
  ('push_api_url', 'https://quickscanz.com'),
  ('cron_secret',  'REPLACE_WITH_YOUR_CRON_SECRET')
on conflict (key) do nothing;

-- RLS: only service role can read/write (cron runs as service role)
...
create policy "app_config_service_only" on public.app_config
  for all using (false);  -- no anon/authed access; service_role bypasses RLS


-- ── 3. SUPPORT TABLES FOR PUSH NOTIFICATIONS ────────────────
-- Push subscriptions table (stores browser push endpoints)
...
-- ── 4. REWRITTEN CRON FUNCTION ──────────────────────────────
-- Reads push_api_url and cron_secret from app_config table
-- instead of database GUC parameters (no ALTER DATABASE needed).
--
-- NOTE: requires pg_net and pg_cron extensions.
-- Enable them in Supabase Dashboard → Database → Extensions
-- (search "pg_net" and "pg_cron" → toggle on)
...
declare
  ...
  v_secret   text;
begin
  -- Read config from app_config table (no superuser needed)
  ...
  -- Skip if config not yet populated
  if v_api_url is null or v_secret is null or v_secret = 'REPLACE_WITH_YOUR_CRON_SECRET' then
    raise notice 'notify_expiring_warranties: app_config not set, skipping';
    return;
  end if;

  -- Fire HTTP call to /api/push/send for each expiry window
  ...
  perform net.http_post(
    ...
    body    := jsonb_build_object('days_before', days)
  )
  from unnest(array[1, 7, 30]) as days;
end;
$$;

-- Schedule: daily at 9am IST (3:30 UTC)
...
-- ── 5. FAMILY TABLES (if not already created) ───────────────
...
-- ── 6. SUBSCRIPTION PLANS (seed if empty) ───────────────────
...
insert into public.subscription_plans (id, name, description, price_inr, interval, product_limit, features) values
  ...
  ('pro_monthly', 'Pro', 'Unlimited tracking for your whole home', 149, 'monthly', 999999,
   '["Unlimited products","Unlimited AI claims","Family Vault","Invoice storage","WhatsApp share","Priority support"]'::jsonb),
  ('pro_yearly', 'Pro', 'Best value — save ₹789 vs monthly', 999, 'yearly', 999999,
   '["Unlimited products","Unlimited AI claims","Family Vault","Invoice storage","WhatsApp share","Priority support","Save ₹789/yr"]'::jsonb)
...
-- ── 7. GET_FAMILY_MEMBERS_WITH_EMAIL RPC ─────────────────────
